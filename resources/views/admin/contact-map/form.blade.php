...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ isset($model) ? route('contact-map.update',$model->id) :  route('contact-map.store')}}"
                        ...
                        <div class="tab-content" id="custom-tabs-one-tabContent">
                            @foreach(config('app.languages') as $lang)
                                <div class="tab-pane fade {{$loop->first ? 'active show' : ''}}" id="tab-{{$lang}}"
                                    ...
                                    <div class="form-group">
                                        <label>Phone</label>
                                        <input type="text" placeholder="Phone" name="{{$lang}}[phone]"
                                               value="{{old($lang.'phone', isset($model) ? $model->translateOrDefault($lang)->phone : '' )}}"
                                               class="form-control">
                                        @error("$lang.phone")
                                        <span class="text-danger">{{$message}}</span>
                                        @enderror
                ...
                <div class="form-group">
                    <label>Iframe</label>
                    <input type="text" placeholder="Iframe" name="iframe" value="{{old('iframe',$model->iframe ?? '')}}" class="form-control">
                    @error('iframe')
                    <span class="text-danger">{{$message}}</span>
                    @enderror
                </div>
